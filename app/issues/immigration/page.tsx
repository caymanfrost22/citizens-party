...
const VISA_DATA = [
  { visa: 'H-1B', purpose: 'Specialty occupations (tech, finance, engineering)', cap: '65,000 + 20,000 masters', view: 'Reform', viewColor: 'var(--gold)', viewBg: 'color-mix(in srgb, var(--gold) 15%, transparent)', note: 'Raise wage floor to market rate; strengthen American worker priority; cap body-shop firms' },
  { visa: 'H-2A', purpose: 'Temporary agricultural workers', cap: 'No cap', view: 'Keep', viewColor: 'var(--green)', viewBg: 'color-mix(in srgb, var(--green) 15%, transparent)', note: 'Essential to food production; well-structured with return requirement' },
  { visa: 'H-2B', purpose: 'Temp non-agricultural (hospitality, landscaping)', cap: '66,000 + 64,716 supplemental', view: 'Keep/Review', viewColor: 'var(--teal)', viewBg: 'color-mix(in srgb, var(--teal) 15%, transparent)', note: 'Cap supplemental at base level; enforce return requirement strictly' },
  { visa: 'L-1', purpose: 'Intracompany transfers (managers, specialists)', cap: 'No cap', view: 'Keep', viewColor: 'var(--green)', viewBg: 'color-mix(in srgb, var(--green) 15%, transparent)', note: 'Tighten "specialized knowledge" definition to prevent abuse' },
  { visa: 'O-1', purpose: 'Extraordinary ability (arts, science, business)', cap: 'No cap', view: 'Keep', viewColor: 'var(--green)', viewBg: 'color-mix(in srgb, var(--green) 15%, transparent)', note: 'High bar; attracts genuine top global talent' },
  { visa: 'TN', purpose: 'USMCA professionals (Canada & Mexico)', cap: 'No cap', view: 'Keep', viewColor: 'var(--green)', viewBg: 'color-mix(in srgb, var(--green) 15%, transparent)', note: 'Trade agreement obligation; practical and well-managed' },
...
const PILLARS = [
  ...
  {
    num: '02', color: 'var(--gold)', title: 'Merit-Based Legal Immigration — The Canada Model',
  ...
  {
    num: '03', color: 'var(--teal)', title: 'Work Visa Reform — Protect American Workers',
  ...
  {
    num: '04', color: 'var(--green)', title: 'Strengthen Citizenship — Honor the Oath',
  ...
  {
    num: '05', color: 'var(--gold)', title: 'Undocumented Population — Rule of Law, No Amnesty',
  ...
  return (
    <div>
      ...
      <div className="py-20 px-4 text-center" style={{ background: 'linear-gradient(135deg, var(--navy), var(--card))' }}>
        <SectionLabel color="var(--teal)" bg="color-mix(in srgb, var(--teal) 10%, transparent)" border="color-mix(in srgb, var(--teal) 30%, transparent)">🛡️ Immigration Reform</SectionLabel>
        <h1 className="text-5xl md:text-6xl font-black mb-4">
          Secure Borders.<br />
          <span style={{ color: 'var(--gold)' }}>Smart Legal Immigration.</span>
        </h1>
        ...
        <div className="flex flex-wrap gap-3 justify-center">
          {['Secure the Border', 'Merit-Based Selection', 'Protect American Workers', 'Rule of Law', 'No Amnesty'].map(tag => (
            <span key={tag} className="px-3 py-1 rounded-full text-xs font-bold"
              style={{ background: 'color-mix(in srgb, var(--teal) 15%, transparent)', border: '1px solid color-mix(in srgb, var(--teal) 40%, transparent)', color: 'var(--teal)' }}>
              {tag}
            </span>
          ))}
        </div>
      ...
      <div className="py-8 px-4" style={{ background: '#0d1f38', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <StatCard num="1M+" label="Legal immigrants per year" color="var(--gold)" />
          ...
          <StatCard num="65K" label="H-1B base cap per year" color="var(--gold)" />
          <StatCard num="5 yrs" label="Residency for citizenship" color="var(--teal)" />
      ...
      <div className="max-w-5xl mx-auto px-4 py-16 space-y-20">
        ...
        <section>
          <div className="text-center mb-10">
            ...
            <p className="text-[var(--muted)] max-w-xl mx-auto">Title 8, United States Code — the primary statute governing all US immigration since 1952.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {[
              ...
              <div key={item.title} className="p-6 rounded-2xl" style={{ background: 'var(--card)', border: '1px solid color-mix(in srgb, var(--teal) 20%, transparent)' }}>
                <div className="text-3xl mb-3">{item.icon}</div>
                <h3 className="font-bold mb-2" style={{ color: 'var(--teal)' }}>{item.title}</h3>
                <p className="text-sm mb-3" style={{ color: 'var(--muted)' }}>{item.desc}</p>
                <div className="text-xs font-mono" style={{ color: 'var(--muted)', opacity: 0.7 }}>{item.cite}</div>
              </div>
            ))}
          </div>
        ...
        <section>
          <div className="text-center mb-10">
            ...
            <p className="text-[var(--muted)] max-w-xl mx-auto">Five main channels admit 1M+ people per year. Two of them have serious structural problems.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            {[
              {
                icon: '👨‍👩‍👧‍👦', title: 'Family-Based (~65% of green cards)', color: 'var(--gold)', border: 'color-mix(in srgb, var(--gold) 30%, transparent)',
              ...
              {
                icon: '💼', title: 'Employment-Based (~14% of green cards)', color: 'var(--green)', border: 'color-mix(in srgb, var(--green) 30%, transparent)',
              ...
              <div key={item.title} className="p-6 rounded-2xl" style={{ background: 'var(--card)', border: `1px solid ${item.border}` }}>
                <div className="text-3xl mb-2">{item.icon}</div>
                <h3 className="font-bold mb-2" style={{ color: item.color }}>{item.title}</h3>
                <p className="text-sm mb-3" style={{ color: 'var(--muted)' }}>{item.desc}</p>
                <ul className="text-sm space-y-1 mb-3">
                  {item.sub.map(s => <li key={s} style={{ color: 'var(--muted)' }}>• {s}</li>)}
                </ul>
                <div className="text-xs font-mono mb-3" style={{ color: 'var(--muted)', opacity: 0.7 }}>{item.cite}</div>
                {item.warn && (
        ...
        <section>
          <div className="text-center mb-10">
            ...
            <p className="text-[var(--muted)] max-w-xl mx-auto">Temporary authorization to work in the US — most require employer sponsorship.</p>
          </div>
          <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="px-5 py-3 hidden md:block" style={{ background: '#0d1f38' }}>
              <div className="grid grid-cols-5 gap-4 text-xs font-bold uppercase tracking-widest" style={{ color: 'var(--muted)' }}>
                <span>Visa</span><span>Purpose</span><span>Annual Cap</span><span>Our View</span><span>Reform Note</span>
              </div>
            </div>
            {VISA_DATA.map((row, i) => (
              <div key={row.visa} className="p-4 md:p-0" style={{ background: i % 2 === 0 ? 'var(--card)' : '#162236', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
                {/* Mobile */}
                <div className="md:hidden space-y-1">
                  <div className="flex items-center gap-3">
                    <span className="font-black text-lg" style={{ color: 'var(--gold)' }}>{row.visa}</span>
                    <span className="px-2 py-0.5 rounded-full text-xs font-bold" style={{ background: row.viewBg, color: row.viewColor }}>{row.view}</span>
                  </div>
                  <div className="text-sm" style={{ color: 'var(--muted)' }}>{row.purpose}</div>
                  <div className="text-xs" style={{ color: 'var(--muted)', opacity: 0.7 }}>Cap: {row.cap}</div>
                ...
                <div className="hidden md:grid grid-cols-5 gap-4 px-5 py-4 text-sm items-start">
                  <span className="font-black" style={{ color: 'var(--gold)' }}>{row.visa}</span>
                  <span style={{ color: 'var(--muted)' }}>{row.purpose}</span>
                  <span style={{ color: 'var(--muted)' }}>{row.cap}</span>
                  ...
                  <span className="text-xs" style={{ color: 'var(--muted)' }}>{row.note}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-3 p-4 rounded-2xl" style={{ background: 'color-mix(in srgb, var(--gold) 6%, transparent)', border: '1px solid color-mix(in srgb, var(--gold) 20%, transparent)' }}>
            <p className="text-sm" style={{ color: 'var(--muted)' }}>
        ...
        <section>
          <div className="text-center mb-10">
            <SectionLabel color="var(--green)" bg="color-mix(in srgb, var(--green) 10%, transparent)" border="color-mix(in srgb, var(--green) 30%, transparent)">🇺🇸 Citizenship</SectionLabel>
            <h2 className="text-4xl font-black mb-3">Path to American Citizenship</h2>
            <p className="text-[var(--muted)] max-w-xl mx-auto">Current requirements under 8 U.S.C. §§ 1401–1429 — the naturalization statute.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-6 rounded-2xl" style={{ background: 'var(--card)', border: '1px solid color-mix(in srgb, var(--green) 20%, transparent)' }}>
              <h3 className="font-bold mb-4" style={{ color: 'var(--green)' }}>Standard Naturalization Requirements</h3>
              <div className="space-y-3">
                {[
                  ...
                  <div key={item.label} className="flex gap-3">
                    <div className="w-2 h-2 rounded-full mt-1.5 shrink-0" style={{ background: 'var(--green)' }}></div>
                    <div>
                      <span className="font-semibold text-white">{item.label}: </span>
                      <span className="text-sm" style={{ color: 'var(--muted)' }}>{item.val}</span>
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-4 text-xs" style={{ color: 'var(--muted)', opacity: 0.7 }}>8 U.S.C. § 1427; Form N-400 ($640 fee)</div>
            </div>
            <div className="space-y-4">
              <div className="p-6 rounded-2xl" style={{ background: 'var(--card)', border: '1px solid rgba(200,16,46,0.3)' }}>
              ...
              <div className="p-6 rounded-2xl" style={{ background: 'var(--card)', border: '1px solid rgba(255,255,255,0.07)' }}>
                <h3 className="font-bold mb-3" style={{ color: 'var(--gold)' }}>Accelerated Paths to Citizenship</h3>
                ...
                <div className="mt-4 p-3 rounded-xl text-sm" style={{ background: 'color-mix(in srgb, var(--gold) 8%, transparent)', border: '1px solid color-mix(in srgb, var(--gold) 20%, transparent)', color: 'var(--gold)' }}>
        ...
        <section>
          <div className="text-center mb-10">
            ...
            <p className="text-[var(--muted)] max-w-xl mx-auto">Statutory and regulatory authority — and where it has been failing.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {[
              ...
              { icon: '📝', title: 'Asylum Credible Fear', desc: 'Applicants must show a "significant possibility" of establishing asylum eligibility. Immigration judges review officer determinations de novo.', color: 'var(--gold)', cite: 'FR Doc. 2024-30500 (Dec 2024)' },
            ].map(item => (
              <div key={item.title} className="p-5 rounded-2xl" style={{ background: 'var(--card)', border: `1px solid ${item.color === '#c8102e' ? 'rgba(200,16,46,0.3)' : 'color-mix(in srgb, var(--gold) 30%, transparent)'}` }}>
                <div className="text-3xl mb-2">{item.icon}</div>
                <h3 className="font-bold mb-2" style={{ color: item.color }}>{item.title}</h3>
                <p className="text-sm" style={{ color: 'var(--muted)' }}>{item.desc}</p>
                {item.cite && <div className="mt-2 text-xs font-mono" style={{ color: 'var(--muted)', opacity: 0.7 }}>{item.cite}</div>}
              </div>
            ))}
          </div>
        ...
        <section>
          <div className="text-center mb-10">
            ...
            <p className="text-[var(--muted)] max-w-xl mx-auto">Secure borders. Merit-based legal immigration. Consistent rule of law.</p>
          </div>
          <div className="space-y-6">
            {PILLARS.map(pillar => (
              <div key={pillar.num} className="p-6 rounded-2xl" style={{ background: 'var(--card)', border: `1px solid ${pillar.color}30` }}>
        ...
        <section>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="hidden md:grid grid-cols-3 gap-0 px-5 py-3" style={{ background: '#0d1f38' }}>
              <div className="text-xs font-bold uppercase tracking-widest" style={{ color: 'var(--gold)' }}>Issue</div>
              <div className="text-xs font-bold uppercase tracking-widest" style={{ color: '#c8102e' }}>Current System</div>
              <div className="text-xs font-bold uppercase tracking-widest" style={{ color: 'var(--green)' }}>Citizens Party Reform</div>
            </div>
            {COMPARE_ROWS.map((row, i) => (
              <div key={row.issue} className="grid grid-cols-1 md:grid-cols-3 gap-0" style={{ background: i % 2 === 0 ? 'var(--card)' : '#162236', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
                ...
                <div className="px-5 py-4 text-sm font-semibold" style={{ color: 'var(--green)' }}>{row.reform}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          <div className="text-center mb-10">
            ...
            <p className="text-[var(--muted)] max-w-xl mx-auto">Real answers, not talking points.</p>
          </div>
        ...
        {/* Legal Citations */}
        <section>
          <div className="p-5 rounded-2xl" style={{ background: 'var(--card)', border: '1px solid rgba(255,255,255,0.07)' }}>
            <h3 className="font-bold mb-3 text-sm" style={{ color: 'var(--muted)' }}>Legal Citations</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-1 text-xs font-mono" style={{ color: 'var(--muted)', opacity: 0.7 }}>
              {[
