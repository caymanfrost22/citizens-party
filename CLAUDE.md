...
| **4 (NEXT)** | Healthcare access (Medicare, single-payer) | not started | mid sensitivity — distinct from `/issues/healthcare-costs` (Econ pillar). Note: a `Healthcare` social row already exists in `platform_positions` (priority 9) |
...
**Resume protocol:** next is Healthcare access (Medicare, single-payer) — distinct from the Economics-pillar `/issues/healthcare-costs` page (which is cost/pricing); this is access/coverage/single-payer. A `Healthcare` social row already exists in `platform_positions` (priority 9) — UPDATE it, don't insert a dupe. Follow the methodology established in Governance pillar (see canonical section at bottom) — present D/R positions + rhetoric vs. behavior, lay out full option space across every dimension, give recommended picks, wait for user picks per dimension, then write page + DB row + hub one-liner. When adding a sub-issue to the Social hub, move it from the `UPCOMING` chips array into `SUB_ISSUES` in `app/issues/social/page.tsx`.
