...
**Resume protocol:** next is Drug policy (fentanyl, cannabis). Follow the methodology established in Governance pillar (see canonical section at bottom) — present D/R positions + rhetoric vs. behavior, lay out full option space across every dimension, give recommended picks, wait for user picks per dimension, then write page + DB row + hub one-liner. When adding a sub-issue to the Social hub, move it from the `UPCOMING` chips array into `SUB_ISSUES` in `app/issues/social/page.tsx`. Note: drug-policy criminal-justice overlap (treatment courts, psychedelic therapy) already partly covered on `/issues/crime` — the drug page carries fentanyl enforcement + cannabis legalization/scheduling specifics.
