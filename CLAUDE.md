...
**Resume protocol:** TWO REMAINING, both HIGH sensitivity — Second Amendment / guns (#5) then Abortion (#6). Careful framing required; expect the user to want max-defensible framing on the constitutional landmines (pattern this session: pledge/Barnette, Christian-heritage/Establishment-Clause, forced-military/13th-Amendment, restitution fairness). Follow the methodology (canonical section at bottom) — present D/R rhetoric vs. behavior, full option space per dimension, recommended picks, WAIT for user picks per dimension, then write page + DB row + hub one-liner. When adding to the Social hub, move it from the `UPCOMING` chips array into `SUB_ISSUES` in `app/issues/social/page.tsx`.
