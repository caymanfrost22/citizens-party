...
export default function ThreeWayCompare({ dem, rep, us }: ThreeWayCompareProps) {
  return (
    ...
    >
      ...
      <div className="px-5 py-5 rounded-br-2xl" style={{ background: 'color-mix(in srgb, var(--green) 8%, transparent)' }}>
        <div className="text-xs font-bold mb-2" style={{ color: 'var(--green)' }}>🟢 We Say</div>
        <p className="text-sm font-semibold" style={{ color: 'var(--green)' }}>{us}</p>
      </div>
    </div>
  )
}
